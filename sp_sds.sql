...
if object_id('sp_sds','P') is not null
  drop procedure sp_sds;
go
...
IF UPPER(ISNULL(@Level, 'DATABASE')) = 'FILE'
  BEGIN
	...
	if @OutputType = 'Analysis'
      begin
        if object_id('TempDB..##sds_file','U') is not null
          drop table ##sds_file;
  ...
  BEGIN
    ...
    IF @Unit = 'KB'
     ...
     IF @Unit = 'GB'
	  ...
	  if @OutputType = 'HumanReadable'
		SELECT CONVERT(dec(10, 2), TOTAL*100.0/@GrantTotal) AS 'WEIGHT (%)', 
		...
		begin
		  if object_id('TempDB..##sds_db','U') is not null
		    drop table ##sds_db;
    ...
    IF @TargetDatabase IS NULL and @OutputType = 'HumanReadable'
      SELECT CASE WHEN @Unit = 'GB' THEN 'GB' WHEN @Unit = 'KB' THEN 'KB' ELSE 'MB' END AS 'SUM', 
      SUM (used) AS 'USED', 
      SUM (free) AS 'FREE', 
      SUM (TOTAL) AS 'TOTAL', 
